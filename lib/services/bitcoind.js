...
/**
 * Provides a friendly event driven API to paccoind in Node.js. Manages starting and
 * stopping paccoind as a child process for application support, as well as connecting
 * to multiple paccoind processes for server infrastructure. Results are cached in an
...
Bitcoin.prototype.getAPIMethods = function() {
  var methods = [
    ...
    ['getSupplyInfo', this, this.getSupplyInfo, 1]
  ];
  return methods;
};
...
Bitcoin.prototype._loadSpawnConfiguration = function(node) {
  ...
  $.checkArgument(this.options.spawn, 'Please specify "spawn" in paccoind config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in paccoind config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in paccoind config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './paccoin.conf');
...
Bitcoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    ...
    '"txindex" option is required in order to use transaction query features of bitcore-node-pac. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of bitcore-node-pac. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of bitcore-node-pac. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that paccoind is undergoing a reindex. ' +
             ...
             'of bitcore-node-pac services will start.');
    node._reindex = true;
  }
};
...
Bitcoin.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/paccoin.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/paccoin.conf';
    }
  }
  return networkPath;
};
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('PAC Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Bitcoin.prototype._checkReindex = function(node, callback) {
  ...
  interval = setInterval(function() {
    node.client.getBlockchainInfo(function(err, response) {
      ...
      log.info('PAC Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
...
Bitcoin.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/paccoind.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned PAC process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean PAC process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
Bitcoin.prototype._spawnChildProcess = function(callback) {
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('PAC process unexpectedly exited with code:', code);
        log.warn('Restarting PAC child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('PAC process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn paccoind.'));
      }
...
Bitcoin.prototype._connectProcess = function(config, callback) {
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to paccoind.'));
    }
 ...
 * @param {Object} txid - A $PAC transaction id
 * @param {Function} callback
 */
    ...
    for (var i = 0; i < gobjects.length; i++) {
      ...
      if ((options.type && proposal.type === options.type)) {
          result.push({
              ...
              CollateralHash: response.result[gobjects[i]].CollateralHash,
              CreationTime: response.result[gobjects[i]].CreationTime,
...
Bitcoin.prototype.getMNList = function(callback){
  ...
  //DEACTIVATED because of slow response from paccoind
  ...
  var prepareResponse = function(err){
    ...
    if(
        ...
        for(var i = 0; i<rankLength; i++){
	      ...
	      lastpaidtime:rawResults['lastpaidtime'][vin],
  ...
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('paccoind spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('paccoind process did not exit'));
      }
...
Bitcoin.prototype.getSupplyInfo = function(callback) {
  var self = this;
  var rawResult = {};
  ...
  var getLockedInMasternodes = function(next){
    self.client.masternodelist('status', function(err, response){
      if(response && response.hasOwnProperty('result')){
        var result = response.result;
        var mnCollateral = 500000;
        var mnCount = Object.keys(result).length;
        rawResult.lockedinmasternodes = mnCount * mnCollateral;
      }
      next();
    });
  };

  var getCurrentSupply = function(next){
    self.client.getTxOutSetInfo(function(err, response){
      if(response && response.hasOwnProperty('result')){
        rawResult.currentsupply = response.result.total_amount;
      }
      next();
    });
  };
  ...
  var prepareResponse = function(err){
    ...
    if (keys.indexOf('currentsupply') > -1 && keys.indexOf('lockedinmasternodes')> -1){
      return callback(null, rawResult);
    } else {
      return callback(new Error('Invalid Supply Info'), null);
    }
  };

  return async.series([checkSync, getLockedInMasternodes, getCurrentSupply], prepareResponse);
};

Bitcoin.prototype.govObjectSubmit = function(parentHash, revision, time, dataHex, collateral, callback) {
  var self = this;
  this.client.gobject('submit', parentHash, revision, time, dataHex, collateral, function (err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
    }
    return callback(null, response.result);
